...
class Magic {
     ...
     * @param {boolean} [ignoreSeparation] - whether to ignore the separation or not
     * @return {null|*[]|string[]|*}
     */
    static parsePath(path, separator, ignoreSeparation) {
        if (!path) return null;
     ...
     * @return {*}
     */
    static get(obj, path, separator, ignoreParameters) {
        if (!obj) throw new Error('Invalid object for get');
        if (!$check.object(obj) && !$check.array(obj)) throw new Error('Invalid object for get');
        return jsonPointer.get(obj, Magic.parsePath(path, separator, ignoreParameters));
    }
